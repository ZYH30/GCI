...
quant_t = np.quantile(moniData['t'],[0,0.33,0.67,1])
for i in range(head_num):
...
for i in range(100):
    ...
    DrnetM_predict_delta = DrnetM.predict([input_t - 0.1, input_c,input_t_head])
    predict_MTEF = (DrnetM_predict - DrnetM_predict_delta) / 0.1
    ...
    train_RMSE_MSE.append(round(np.sqrt(mean_squared_error(DrnetM_predict, y_true)),4))
    ...
    DrnetM_predict_test_delta_ = DrnetM.predict([input_t_test - 0.1, input_c_test,input_t_head_test])
    predict_MTEF_test = (DrnetM_predict_test - DrnetM_predict_test_delta_) / 0.1
    ...
    test_RMSE_MSE.append(round(np.sqrt(mean_squared_error(DrnetM_predict_test, y_true_test)),4))
