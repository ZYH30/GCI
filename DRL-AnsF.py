# -*- coding: utf-8 -*-
"""
Created on Thu Mar  7 18:57:14 2024
...
def splitData(data, x_name, x_dim = 5):
    ...
    return input_t,input_c,input_y,y_true,y_delta,input_t_test,input_c_test,input_y_test,y_true_test,y_delta_test 



input_t,input_c,input_y,y_true,y_delta,input_t_test,input_c_test,input_y_test,y_true_test,y_delta_test = splitData(moniData, x_name)
...
c_dim = x_dim
y_dim = 1
x_intermediate_dim = round(x_dim/2 + 2)
...
# d_model.summary()
# plot_model(d_model,to_file='gan_discriminator.png', show_shapes=True)
...
# g_model.summary()
# plot_model(g_model,to_file='gan_generator.png', show_shapes=True)
...
for i in range(100):
    ...
    # Start training
    history = wgan.fit([input_t, input_c,input_y],batch_size=batch_size, epochs=epochs, verbose = 0)
    ...
    predict = wgan.predict([input_t, input_c])
    ...
    GSCM_predict_delta = wgan.predict([input_t - 1, input_c])[1]
    predict_MTEF = GSCM_predict - GSCM_predict_delta
    ...
    predict_test = wgan.predict([input_t_test, input_c_test])
    ...
    GSCM_predict_test_delta_ = wgan.predict([input_t_test - 1, input_c_test])[1]
    predict_MTEF_test = GSCM_predict_test - GSCM_predict_test_delta_
